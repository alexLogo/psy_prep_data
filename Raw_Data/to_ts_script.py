...
if __name__ == "__main__":
    idx_counter = 1
    log = ""
    for dir_name in os.listdir(cfg.raw_data_path):
        # checking if we are in a directory 
        path = os.path.join(cfg.raw_data_path, dir_name)
        ...
        path = cfg.ts_data_path+"participant"+str(idx_counter)+".csv"
        ts_data.to_csv(path)
        idx_counter += 1
    
    # write log into file
    log_path = os.path.join(cfg.ts_data_path, "log.txt")
    f = open(log_path, "w")
    f.write(log)
    f.close()
